...
public class GPConnection {
	...
	private void installCapFile(String _appletUrl, byte[] params, byte privileges)
		...
		if (is == null) {
			throw new IOException("Failed to open file: InputStream is null");
		}

		CapFile cpFile = new CapFile(is, null);
		AID p = cpFile.getPackageAID();
		if (p == null) {
			throw new IOException("Failed to read package AID from CAP file. Invalid CAP?");
		}

		try {
			Log.d(LOG_TAG, "Attempting to delete existing package " + p.toString());
			for (AID a : cpFile.getAppletAIDs()) {
				try {
					mGPService.deleteAID(a, false);
				} catch (Exception ignored) {}
			}
			mGPService.deleteAID(p, true);
			Log.d(LOG_TAG, "Successfully deleted existing package");
		} catch (Exception e) {
			Log.d(LOG_TAG, "Package deletion failed (probably not installed): " + e.getMessage());
		}
	...
	public String performCommand(CardChannel channel, GPKeyset keyset,
		...
		try {
			...
			e.printStackTrace();
			ret = "Exception " + e.getLocalizedMessage();
		}
		return ret;
	}
